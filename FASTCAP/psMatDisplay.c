...
{
  // local var fp renamed and made global
  //static FILE *fpstat = NULL;
  double widx, widy;
  ...
  if(type == OPEN) {
    ...
    if((fpstat = fopen(filename, "w")) == NULL) {
      viewprintf(stderr, "dump_ps_mat: can't open `%s' to write\n", filename);
    ...
    dump_aldus_hdr(fpstat, OFFSETX, OFFSETY, OFFSETX+widx, OFFSETY+widy, TRUE);

    /* setup lines to be of the proper size and shape */
    fprintf(fpstat, "%g setlinewidth %d setlinecap %d setlinejoin %d setgray\n",
	    blk_size, LINCAP, LINJIN, GREYLV);

    /* dump the line definition */
    fprintf(fpstat, "/L {2 copy moveto exch %g add exch lineto stroke} def\n",
	    blk_size);

  }
  else if(type == UPDATE) {
    if(fpstat == NULL) {
      viewprintf(stderr, "dump_ps_mat: ps file not open\n");
      FCExit(FC_GENERIC_ERROR);
    }

    /* write a block in the (row, col) position
    dump_line(fpstat, OFFSETX+(blk_size*(double)col), 
	...
	if(aliased_matrix[i][j] == '1') {
	  dump_line(fpstat, OFFSETX+(blk_size*(double)j), 
    ...
    dump_aldus_foot(fpstat, TRUE, argv, argc, TRUE, str);
    fclose(fpstat);
  }
  else {
    viewprintf(stderr, "dump_ps_mat: bad type of call\n");
    FCExit(FC_GENERIC_ERROR);
  }
