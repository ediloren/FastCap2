...
surface **surf_list;
{
  ...
  if((fp = fopen(list_file, "r")) == NULL) {
    viewprintf(stderr, "read_list_file: can't open list file\n  `%s'\nto read\n", 
	    list_file);
  ...
  while(fgets(tline, sizeof(tline), fp) != NULL) {
	...
	viewprintf(stderr,"read_list_file: bad group name format, line %d:\n%s\n",
		linecnt, tline);
	FCExit(FC_GENERIC_ERROR);
      }
    }
	...
	else if(isblankline(tline) == TRUE)
		; // ignore blank lines, Enrico 2002/02/17
    else {
      viewprintf(stderr, "read_list_file: bad line format, line %d:\n%s\n", 
		linecnt, tline);
      FCExit(FC_GENERIC_ERROR);
    }
  }
...
charge *panel_list;
{
  ...
  for(nc = panel_list; nc != NULL; nc = nc->next) {
	...
	viewprintf(stderr, "  Reference point: (%g %g %g)\n",
		ref[0], ref[1], ref[2]);
	viewprintf(stderr, "  Panel cntr: (%g %g %g)\n",
		nc->x, nc->y, nc->z);
...
*/
//  ^
//  |  that means that searches for a name to be given to the .ps output files; 
// (with the -q option, charge distribution ps file is generated; so it needs a name)
// the name is UNIQUE; the criteria followed is: list file name if exists, if not 
// the file name, if not, "stdin"
...
surface *surf_list;
{
  ...
  for(cur_surf = surf_list; cur_surf != NULL; cur_surf = cur_surf->next) {
    if(!strcmp(cur_surf->name, "stdin")) {
      if(stdin_read) {
	viewprintf(stderr, "read_panels: attempt to read stdin twice\n");
	FCExit(FC_GENERIC_ERROR);
      }
      else {
	stdin_read = TRUE;
	fp = stdin;
      ...
      viewprintf(stderr, "read_panels: can't open\n  `%s'\nto read\n", 
	      cur_surf->name);
      FCExit(FC_GENERIC_ERROR);
    }
	...
	viewprintf(stderr, "read_panels: bad panel shape, %d\n",
		cur_panel->shape);
	FCExit(FC_GENERIC_ERROR);
      }
...
char *argv[], **input_file, **surf_list_file;
{
...
#if CAPVEW == ON
  extern int s_, n_, g_, c_, x_, k_, rc_, rd_, rb_, q_, rk_, m_, f_, dd_, z_;
  ...
  s_ = n_ = g_ = c_ = x_ = k_ = rc_ = rd_ = rb_ = q_ = rk_ = m_ = f_ = z_ = FALSE;
  dd_ = FALSE;
#endif
    ...
    if(argv[i][0] == '-') {
      if(argv[i][1] == 'o') {
	    *numMom = (int) strtol(&(argv[i][2]), chkp, 10);
	      if(*chkp == &(argv[i][2]) || *numMom < 0) {
	        viewprintf(stderr, "%s: bad expansion order `%s'\n", 
		               argv[0], &argv[i][2]);
	        cmderr = TRUE;
	        break;
		  }
	    ...
	    *numLev = (int) strtol(&(argv[i][2]), chkp, 10);
	    if(*chkp == &(argv[i][2]) || *numLev < 0) {
	      viewprintf(stderr, "%s: bad partitioning depth `%s'\n", 
		    argv[0], &argv[i][2]);
	      cmderr = TRUE;
	      break;
		}
	    ...
	    if(cmderr) {
	      viewprintf(stderr, "%s: bad permittivity `%s'\n", argv[0], &argv[i][2]);
	      break;
		}
      }
      else if(argv[i][1] == 'l') {
	    ...
	    *read_from_stdin = TRUE;
        ...
        if(sscanf(&(argv[i][2]), "%lf", &azimuth) != 1) {
	      viewprintf(stderr, "%s: bad view point azimuth angle '%s'\n", 
		    argv[0], &argv[i][2]);
	      cmderr = TRUE;
	      break;
		}
        ...
        if(sscanf(&(argv[i][2]), "%lf", &elevation) != 1) {
	      viewprintf(stderr, "%s: bad view point elevation angle '%s'\n", 
		    argv[0], &argv[i][2]);
	      cmderr = TRUE;
	      break;
		}
        ...
        if(sscanf(&(argv[i][2]), "%lf", &iter_tol) != 1 || iter_tol <= 0.0) {
	      viewprintf(stderr, "%s: bad iteration tolerence '%s'\n", 
		    argv[0], &argv[i][2]);
	      cmderr = TRUE;
	      break;
		}
      ...
      /*else if(!strcmp(&(argv[i][1]), "rb")) rb_ = TRUE;*/
        ...
        if(sscanf(&(argv[i][2]), "%lf", &rotation) != 1) {
	      viewprintf(stderr, "%s: bad image rotation angle '%s'\n", 
		    argv[0], &argv[i][2]);
	      cmderr = TRUE;
	      break;
		}
	    ...
	    if(cmderr) {
	      viewprintf(stderr, "%s: bad view point distance '%s'\n", 
		    argv[0], &argv[i][2]);
	      break;
		}
      }
	    ...
	    if(cmderr) {
	      viewprintf(stderr, "%s: bad image scale factor '%s'\n", 
		    argv[0], &argv[i][2]);
	      break;
		}
      }
        ...
        if(sscanf(&(argv[i][2]), "%lf", &linewd) != 1) {
	      ...
	      viewprintf(stderr, "%s: bad line width '%s'\n", 
		    argv[0], &argv[i][2]);
	      cmderr = TRUE;
	      break;
		}
      ...
      /* -x sets up axes of default length, -x<len> uses len as length */
	    ...
	    if(argv[i][2] == '\0') x_ = TRUE;
	      ...
	      if(sscanf(&(argv[i][2]), "%lf", &axeslen) != 1) {
	        ...
	        viewprintf(stderr, "%s: bad axes length '%s'\n", 
		      argv[0], &argv[i][2]);
	        cmderr = TRUE;
	        break;
		  }
	    ...
	    if(!strcmp(&(argv[i][2]), "x") || !strcmp(&(argv[i][2]), "X"))
	      ...
	      viewprintf(stderr, "%s: bad up axis type `%s' -- use x, y or z\n");
          cmderr = TRUE;
          break;
        }
      }
	    ...
	    viewprintf(stderr, "%s: illegal option -- %s\n", argv[0], &(argv[i][1]));
	    cmderr = TRUE;
	    break;
      }
    }
  ...
  if(cmderr == TRUE) {
    ...
    viewprintf(stderr, "DEFAULT VALUES:\n");
    viewprintf(stderr, "  expansion order = %d\n", DEFORD);
    viewprintf(stderr, "  partitioning depth = set automatically\n");
    viewprintf(stderr, "  permittivity factor = 1.0\n");
    viewprintf(stderr, "  iterative loop ||r|| tolerance = %g\n", ABSTOL);
    viewprintf(stderr, "  azimuth = %g\n  elevation = %g\n  rotation = %g\n",
	    DEFAZM, DEFELE, DEFROT);
    viewprintf(stderr, 
	    "  distance = %g (0 => 1 object radius away from center)\n", 
	    DEFDST);
    viewprintf(stderr, "  scale = %g\n  linewidth = %g\n",
	    DEFDST, DEFSCL, DEFWID);
    if(DEFUAX == XI) viewprintf(stderr, "  upaxis = x\n");
    else if(DEFUAX == YI) viewprintf(stderr, "  upaxis = y\n");
    else if(DEFUAX == ZI) viewprintf(stderr, "  upaxis = z\n");
    viewprintf(stderr, "  axeslength = %g\n", DEFAXE);
    viewprintf(stderr, "OPTIONS:\n");
    viewprintf(stderr, "  -   = force conductor surface file read from stdin\n");
    viewprintf(stderr, "  -rs = remove conductors from solve list\n");
    viewprintf(stderr, "  -ri = remove conductors from input\n");
    viewprintf(stderr, 
...
"  -b  = superimpose lines, arrows and dots in .figfile on all .ps pictures\n");
    viewprintf(stderr, "  -m  = switch to dump-ps-picture-file-only mode\n");
    viewprintf(stderr, 
...
"  -dc = display total charges in shaded .ps picture file (use with -q option)\n");
    viewprintf(stderr, "  -c  = print command line in .ps picture file\n");
    viewprintf(stderr, "  -v  = suppress showpage in all .ps picture files\n");
    viewprintf(stderr, "  -n  = number faces with input order numbers\n");
    viewprintf(stderr, "  -f  = do not fill in faces (don't rmv hidden lines)\n");
    viewprintf(stderr, "  -g  = dump depth graph and quit\n");
    viewprintf(stderr, "  -z  = dump solve time and memory usage\n");
#else
    viewprintf(stderr,
    ...
    viewprintf(stderr, "  -z  = dump solve time and memory usage\n");
#endif
    viewprintf(stderr, "  <cond list> = [<name>],[<name>],...,[<name>]\n");
...
double relperm;
{
  ...
  if(read_from_stdin || (input_file == NULL && surf_list_file == NULL)) {
	...
	// Enrico, setting 'outer_perm' to 'relperm' was a FastCap
	// bug which lead to wrong results when a global
	// permittivity factor was specified with the -p option
	// (probably left for first FastCap version which handled
	// only conductors in homogeneous dielectic).
	// As a matter of fact, in mksCapDump() in mulDisplay.c the results
	// are multiplied again for 'relperm', so the results, for non-list files,
	// are multiplyed two times; see also here below.
	// The bug was partially pointed out in http://hana.physics.sunysb.edu/conpan/fastcap.html
  ...
  if(input_file != NULL) {
	...
	// As a matter of fact, in mksCapDump() in mulDisplay.c the results
	// are multiplied again for 'relperm', so the results, for non-list files,
	// are multiplyed two times, see also above.
	// The bug was partially pointed out in http://hana.physics.sunysb.edu/conpan/fastcap.html
...
char *argv[], *infile;
{
  ...
  // Enrico, NULLs surf_list_file and input_file, which are respectively
  // the names of the input file and of the list file (but only one is
  // present at one time)
  surf_list_file = NULL;
  input_file = NULL;
  ...
  // Enrico, add file name in window caption
  if(input_file != NULL) {
	FCSetName(input_file);
    _fullpath(full, input_file, _MAX_PATH);
  }
  else if(surf_list_file != NULL) {
	FCSetName(surf_list_file);
    _fullpath(full, surf_list_file, _MAX_PATH);
  }
  else {
	  viewprintf(stderr, "No valid filename given!");
	  FCExit(FC_GENERIC_ERROR);
  }
...
surface *surf_list;
{
  ...
  for(cur_surf = surf_list; cur_surf != NULL; cur_surf = cur_surf->next) {
    ...
    if(cur_surf == surf_list) viewprintf(stdout, "   %s\n", cur_surf->group_name);
    else if(cur_surf->prev->end_of_chain)
	viewprintf(stdout, "   %s\n", cur_surf->group_name);

    /* write file name */
    viewprintf(stdout, "    %s", hack_path(cur_surf->name));
    if(cur_surf->type == CONDTR) {
      viewprintf(stdout, ", conductor\n");
      viewprintf(stdout, "      title: `%s'\n", cur_surf->title);
      viewprintf(stdout, "      outer permittivity: %g\n", 
	      cur_surf->outer_perm);
    }
    else if(cur_surf->type == DIELEC) {
      viewprintf(stdout, ", dielectric interface\n");
      viewprintf(stdout, "      title: `%s'\n", cur_surf->title);
      viewprintf(stdout, "      permittivities: %g (inner) %g (outer)\n",
	      cur_surf->inner_perm, cur_surf->outer_perm);
    }
    else if(cur_surf->type == BOTH) {
      viewprintf(stdout, ", thin conductor on dielectric interface\n");
      viewprintf(stdout, "      title: `%s'\n", cur_surf->title);
      viewprintf(stdout, "      permittivities: %g (inner) %g (outer)\n",
	      cur_surf->inner_perm, cur_surf->outer_perm);
    }
    else {
      viewprintf(stderr, "dumpSurfDat: bad surface type\n");
      FCExit(FC_GENERIC_ERROR);
    }
    viewprintf(stdout,"      number of panels: %d\n", 
	    cur_surf->num_panels - cur_surf->num_dummies);
    viewprintf(stdout,"      number of extra evaluation points: %d\n",
	    cur_surf->num_dummies);
    viewprintf(stdout,"      translation: (%g %g %g)\n", 
...
Name **name_list;
{
  ...
  *autlev = OFF;
  ...
  viewprintf(stdout, "Running %s %.1f (%s)\n  Input: %s\n", 
