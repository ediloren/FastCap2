...
int use_ttl_chg;
{
...
#if 1 == 0
  viewprintf(stderr, " %d faces read\n", numfaces);
  viewprintf(stderr, " %d lines read\n", numlines);
  viewprintf(stderr, " average obj point: (%g %g %g), radius = %g\n", 
	  avg[0], avg[1], avg[2], radius);
  viewprintf(stderr, " absolute view point: (%g %g %g)\n",
  ...
  /*viewprintf(stderr, "Starting depth sort...");*/
  ...
  if(g_ == TRUE) {
    ...
    if((fp = fopen(str, "w")) == NULL) {
      viewprintf(stderr, "dump_ps_geometry: can't open\n `%s'\nto write\n", str);
      FCExit(FC_GENERIC_ERROR);
    }
    viewprintf(stdout, "Writing %s...", str);
