...
char *name_suffix;
{
  ...
  fgets(staticline, BUFSIZ, stream);
  if(staticline[0] == '0') {
    *file_is_patran_type = FALSE;
    firstq = quickif(stream, staticline, title, surf_type, trans_vector,
		     num_cond, name_list, name_suffix);
  }
  else {
	/* Enrico, added message informing user when reading PATRAN file */
	viewprintf(stdout, "No title line beginning with '0' found, assuming PATRAN input file\n");

    *file_is_patran_type = TRUE;

    input(stream, staticline, surf_type, trans_vector);
...
int cond_num;
{
  ...
  viewprintf(stdout, "getPatranName: conductor %d has no name\n", cond_num);
  return(NULL);
