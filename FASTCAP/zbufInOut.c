  ...
  while(patch_pntr != NULL) {
	...
	if(patch_pntr->corner[i] == current_grid->ID) {
	  if(i == 0) {
	    ...
	    viewprintf(stderr, "make_charges_from_patches: panel has more than 4 sides\n");
...
FILE *fp;
{
  ...
  while(fgets(linein, sizeof(linein), fp) != NULL) {
	...
	(*tail) = (*tail)->next;
      ...
      if(sscanf(linein,"%lf %lf %lf",&((*tail)->from[0]), &((*tail)->from[1]), 
		&((*tail)->from[2])) != 3) {
	viewprintf(stderr,"readLines: from line %d bad, '%s'\n",flines+1,linein);
	FCExit(FC_GENERIC_ERROR);
      }
      (*tail)->index = *numlines;
      fflag = 0;
  ...
  if(fflag == 0) {
    viewprintf(stderr, "readLines: file ended with unmatched from line\n");
    FCExit(FC_GENERIC_ERROR);
  }
}
...
char *line_file;
{
  ...
  *numlines = 0;
  ...
  if((fp = fopen(line_file, "r")) == NULL) {
    viewprintf(stderr, "getLines: can't open .fig file\n `%s'\nto read\n",
	    line_file);
    FCExit(FC_GENERIC_ERROR);
  }
...
fprintf(fp, "%s%s%s%s%s%s%s", str, str2, str2_1, str3, str3_1, str4, str5);
...
FILE *fp;
{
  ...
  if(fp == NULL) {
    viewprintf(stderr, "dumpLines: null ps file pointer\n");
    FCExit(FC_GENERIC_ERROR);
  }
...
FILE *fp; 
{
  ...
  if(n_ == TRUE) {
    ...
    /*viewprintf(stderr, "Faces and lines numbered\n");*/
  }

  /* if fills were not included, say so
  if(f_) viewprintf(stderr, "Face fills not written to ps file\n"); */
  ...
  if(c_ == TRUE) {			/* print command line if asked for */
    ...
    /*viewprintf(stderr, "Command line printed\n");*/
  }
