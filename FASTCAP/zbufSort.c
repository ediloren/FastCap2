...
double *view;
{
...
#if DEBUGX == ON
  viewprintf(stdout, "Is face %d behind face %d?\n", fac->index, facref->index);
#endif
  is_overlap = FALSE;
  for(i = 0; i < facref->numsides && !is_overlap; i++) {
    ...
    for(j = 0; j < fac->numsides && !is_overlap; j++) {
	  ...
	  is_overlap = TRUE;
#if DEBUGX == ON
      viewprintf(stdout, "doLinesIntersect returned %d\n", intersect);
#endif
    }
  }
  ...
  if(facref->numsides == 4) {
    ...
    viewprintf(stderr, "isThereProjOverlap: can't handle %d side panel\n",
	    facref->numsides);
    FCExit(FC_GENERIC_ERROR);
  }
#else
...
double *view, rhs, *normal;
{
  ...
  if(diff_is_negative(alpha_facref, alpha_fac, 1.0)) return(TRUE);
    ...
    viewprintf(stderr, "  alpha_fac, face %d = %g alpha_facref, face %d = %g\n", 
	    fac->index, alpha_fac, facref->index, alpha_facref);
    dump_face(stderr, fac);
    viewprintf(stderr, " Projected corners\n");
...
double *view, rhs, *normal;
{
  ...
  if(TRUE == TRUE) {		/* if memory can be sacrificed for speed */
	...
	if((check = is1stFaceDeeper(fpcur, fpchk, view, rhs, normal))==TRUE) { 
      ...
      if(f % 20 == 0 && f != 0) {
	viewprintf(stdout, "%d ", f);
	fflush(stdout);
      }
      if(f % 200 == 0 && f != 0) viewprintf(stdout, "\n");
    }
  }
  else {
