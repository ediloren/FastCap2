...
Name **name_list;
{
  ...
  /* name not found */
  viewprintf(stderr, "renameConductor: unknown conductor `%s'\n", old_name);
  return(FALSE);
}
...
FILE *fp;
{
  ...
  while(fgets(line1, sizeof(line1), fp) != NULL) {
    if(line1[0] == 'Q' || line1[0] == 'q') {
	 ...
	 != 14) {
	viewprintf(stderr, "quickif: bad quad format, line %d:\n%s\n", 
		linecnt, line1);
	...
	curquad = curquad->next;
	 ...
	 != 11) {
	viewprintf(stderr, "quickif: bad tri format, line %d:\n%s\n", 
		linecnt, line1);
	FCExit(FC_GENERIC_ERROR);
      }
	...
	curtri = curtri->next;
	  ...
	  // Enrico, corrected bug; original line read 
	  // else curquad->cond = 0;
      else curtri->cond = 0;
	...
	viewprintf(stderr, "quickif: bad rename format, line %d:\n%s\n", 
		linecnt, line1);
	FCExit(FC_GENERIC_ERROR);
      }
	...
	else if(isblankline(line1) == TRUE)
		linecnt++; // ignore blank lines, Enrico 2002/02/17
    else {
      viewprintf(stderr, "quickif: bad line format, line %d:\n%s\n", 
		linecnt, line1);
      FCExit(FC_GENERIC_ERROR);
    }
  }
