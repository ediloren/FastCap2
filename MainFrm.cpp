...
void CMainFrame::OnClose() 
{
	...
	if( g_bIsFCRunning == TRUE ) {
		view = (CFastCapView *) GetActiveView();
		view->OutputText("Error on user 'Close' request: cannot close window while FastCap is running\n", FCV_RED);
	}
	else {
		...
		// for changes saving
		doc = (CFastCapDoc *) GetActiveDocument();
...
afx_msg LRESULT CMainFrame::OnLogText(WPARAM color, LPARAM)
{
	...
	pDoc = (CFastCapDoc *) GetActiveDocument();
...
BOOL CMainFrame::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* pCopyDataStruct) 
{
	...
	// call main function in CFastCapApp
	((CFastCapApp *)AfxGetApp())->OnCopyData(pWnd, pCopyDataStruct);
...
void CMainFrame::OnHelp() 
{
	...
	path = ((CFastCapApp*)AfxGetApp())->m_strAppPath;
